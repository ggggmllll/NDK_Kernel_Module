...
| `include/kmod_uaccess.h` | 声明：`compat_uaccess_init` / `compat_copy_to_user` / `compat_copy_from_user` / `compat_strncpy_from_user` |
...
compat_uaccess_init();   /* 解析内核 copy_to/from_user + strncpy_from_user
                           * + ttbr0_enable/disable */
/* 用户业务 init ... */
```

之后即可用：`klog("...")`、`kallsyms_lookup("printk")`、`compat_copy_from_user(...)` 等。
...
- **uaccess 两层策略**：优先调内核自己的 `copy_to_user` / `copy_from_user` /
  `strncpy_from_user`（`compat_uaccess_init` 解析，最稳）；内核 fn 不可用时
